"use client";

import { Menu } from "lucide-react";
import { useMobileMenu } from "@/components/layout/dashboard-shell";
import { UserMenu } from "@/components/layout/user-menu";
import { useLanguage } from "@/components/i18n/language-provider";

type HeaderProps = {
  title: string;
  description?: string;
  titleAddon?: React.ReactNode;
  actions?: React.ReactNode;
};

export function Header({ title, description, titleAddon, actions }: HeaderProps) {
  const { openMobileMenu } = useMobileMenu();
  const { t } = useLanguage();

  return (
    <header className="sticky top-0 z-40 border-b border-kefoo-200/80 bg-white/80 backdrop-blur-xl">
      <div className="flex items-center justify-between gap-4 px-4 py-5 sm:px-6 lg:px-8">
        <div className="flex min-w-0 items-start gap-3">
          <button
            type="button"
            onClick={openMobileMenu}
            className="mt-0.5 rounded-xl border border-baby-200 bg-white p-2 text-slate-600 shadow-sm hover:bg-baby-50 lg:hidden"
            aria-label={t("header.openMenu")}
          >
            <Menu className="h-5 w-5" />
          </button>
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-3">
              <h1 className="font-heading text-2xl font-semibold tracking-tight text-slate-900">
                {title}
              </h1>
              {titleAddon}
            </div>
            {description && (
              <p className="mt-1 text-sm leading-relaxed text-slate-500">{description}</p>
            )}
          </div>
        </div>
        <div className="flex shrink-0 items-center gap-3">
          {actions}
          <UserMenu />
        </div>
      </div>
    </header>
  );
}
